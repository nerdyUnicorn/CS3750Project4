                            ...
                            <template v-for="item in this.$store.getters.allocations">
                                <tr>
                                    ...
                                    <td class="slider-container">
                                        <input type="range" class="stock-alloc" v-model.number="item.percent" min="0" max="100" v-on:change="limitRange($event, item)"
                                    ...
                                    <td class="number-container">
                                        <input type="number" class="stock-num-box" :disabled="item.percent > 100" v-model.number="item.percent" min="0" max="100"
                                            v-on:change="limitRange($event, item)" v-on:input="limitRange($event, item)" />
                                    </td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
                    <button class="btn submit-port-button" v-bind:disabled="savingPort" v-bind:class="{'btn-info': !savingPort, 'btn-warning': savingPort}"
                        v-on:click="submitPortfolio()">{{saveText}}</button>
                </div>
                <div class="col-xs-6">
                    <div id="chartWindow">
    ...
    export default {
        ...
        methods: {
            ...
            submitPortfolio: function () {
                ...
                this.$http.post('/api/updateAlloc', {
                        portfolio: this.$store.getters.allocations
                    })
                    .then(data => {
